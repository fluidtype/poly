    ...
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/5 bg-[var(--bg)]/90 px-6 backdrop-blur">
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-1">
          ...
          <h1 className="text-xl font-semibold leading-tight text-[var(--fg)]">
      ...
      <nav className="hidden items-center gap-3 text-sm font-medium text-[var(--muted)] lg:flex">
        {links.map((item, index) => (
          <Fragment key={item.label}>
            <a
              className="rounded-full border border-transparent px-3 py-1.5 text-[var(--muted)] transition hover:border-white/10 hover:text-[var(--fg)] focus-visible:border-white/10"
              href={item.href}
            >
              {item.label}
            </a>
      ...
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          className="h-10 w-44 rounded-full border border-white/10 bg-[var(--panel-2)]/70 px-4 text-sm text-[var(--fg)] placeholder:text-[var(--muted)] focus-visible:border-white/20"
          placeholder="Search"
          type="search"
        />
        <button className="rounded-full border border-white/10 bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-[var(--bg)] shadow-[var(--glow)] transition hover:brightness-110">
          New Report
        </button>
      </div>
    </header>
